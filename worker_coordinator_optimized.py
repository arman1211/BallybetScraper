...
def run_pregame_worker():
    """Optimized pregame scraper worker function."""
    from ballybet_scraper_optimized import OptimizedBallyBetScraper
    
    scraper = OptimizedBallyBetScraper(data_dir="ballybet_data")
    scraper.run_continuous(interval=15)  # 15 second cycles (realistic)


def run_live_worker():
    """Optimized live scraper worker function."""
    from ballybet_live_scraper_optimized import OptimizedLiveBallyBetScraper
    
    scraper = OptimizedLiveBallyBetScraper(
        data_dir="ballybet_data",
        poll_interval=6  # 6 second cycles (realistic)
    )
    scraper.run()
